...
Deno.test('sender and recipient are the same', async () => {
    let block_number = 1234;
...
Deno.test('not full withdrawal', async () => {
    let args: TransferArgs = { recipient: '0x4DCeBf483fA7f31FfCee6e4EAffC1D78308Ec2cD', amount: web3.utils.toWei('0.001') };
    let sender = '0xFeaED3f817169C012D040F05C6c52bCE5740Fc37';
...
Deno.test('valid call', async () => {
    let sender = '0xFeaED3f817169C012D040F05C6c52bCE5740Fc37';
    let block_number = await web3.eth.getBlock("latest").then((block) => block.number);
    let balance = await web3.eth.call({ data: glm.methods.balanceOf(sender).encodeABI(), to: glm.options.address }, block_number).then((res) => web3.utils.toBN(res));
