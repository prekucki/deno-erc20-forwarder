...
Deno.test({
    ...
    async fn() {
        ...
        const sender = '0xFeaED3f817169C012D040F05C6c52bCE5740Fc37';
        const block_number = await web3.eth.getBlock('latest').then((block) => block.number);
...
Deno.test({
    name: 'valid call, no block provided',
    ignore: IS_OFFLINE,
    async fn() {
        const sender = '0xFeaED3f817169C012D040F05C6c52bCE5740Fc37';
        const block_number = await web3.eth.getBlock('latest').then((block) => block.number);
        ...
        const error_details = await validateCallArguments(sender, args);
        assertEquals(error_details, undefined);
    },
});
